...
The core binaries (`xray`, `tun2socks`, `sing-box`) and routing data
(`geoip.dat`, `geosite.dat`) are **not stored in git**. Fetch them locally
before packaging:
...
This downloads pinned versions into `bin/arm64-v8a/`, `bin/x86_64/` and `bin/`.
Override versions with `XRAY_VERSION` / `TUN2SOCKS_VERSION` / `SINGBOX_VERSION`
env vars. `sing-box` is the second core, used for Hysteria2/TUIC profiles;
`xray` runs everything else (engine is chosen per profile / per protocol).
