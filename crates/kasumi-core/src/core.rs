...
/// One core-resolution case: a profile and the core Rust resolves it to. Emitted
/// to the frontend so a TS parity test can assert its own [mirror of this logic]
/// agrees (see `frontend/src/lib/profile-utils.ts::resolveCore`). The frontend
/// re-implements the matrix synchronously for the `EngineTag`, so these fixtures
/// are the guard that keeps the two from drifting.
pub struct CoreResolutionFixture {
    pub name: &'static str,
    pub profile: Profile,
    pub expected: CoreEngine,
}

/// A representative sweep of the core-resolution matrix — every forced branch plus
/// the default-table and per-profile-override paths — each paired with the core
/// [`resolve_core`] picks under default settings. The single source of the expected
/// answers (computed here, not restated), consumed by the frontend parity test.
pub fn core_resolution_fixtures() -> Vec<CoreResolutionFixture> {
    use crate::enums::PacketEncoding;
    use crate::profile::empty_profile;
    use crate::share::parse_share_link;

    let link =
        |uri: &str| parse_share_link(uri, Some("g-main")).expect("fixture share link parses");
    let empty = |proto: Protocol| empty_profile(proto, "g-main");

    // (name, profile) — the expected core is filled in by `resolve_core` below.
    let mut cases: Vec<(&'static str, Profile)> = vec![
        // Protocols one core must run.
        ("custom-forced-xray", empty(Protocol::Custom)),
        ("tuic-singbox-only", link("tuic://u:pw@t.ex:443?sni=t.ex")),
        ("anytls-singbox-only", link("anytls://pw@a.ex:443?sni=a.ex")),
        ("naive-singbox-only", empty(Protocol::Naive)),
        ("shadowtls-singbox-only", empty(Protocol::Shadowtls)),
        // Default per-protocol table (nothing forces these).
        (
            "hysteria2-default-singbox",
            link("hysteria2://pw@h.ex:443?sni=h.ex"),
        ),
        (
            "vless-tcp-tls-default-xray",
            link("vless://u@e.x:443?type=tcp&security=tls"),
        ),
        // vmess share links are base64-encoded JSON; an empty vmess profile
        // exercises the same default-table path (vmess → xray).
        ("vmess-tcp-default-xray", empty(Protocol::Vmess)),
        (
            "trojan-tls-default-xray",
            link("trojan://pw@e.x:443?security=tls"),
        ),
        // vless protocol-level forced branches.
        (
            "vless-vision-udp443-xray",
            link("vless://u@e.x:443?type=tcp&security=tls&flow=xtls-rprx-vision-udp443"),
        ),
        (
            "vless-encryption-xray",
            link("vless://u@e.x:443?type=tcp&security=tls&encryption=mlkem768"),
        ),
        // Transport-level forced branches.
        (
            "vless-h2-singbox",
            link("vless://u@e.x:443?type=h2&security=tls"),
        ),
        (
            "vless-quic-singbox",
            link("vless://u@e.x:443?type=quic&security=tls"),
        ),
        ("vless-kcp-xray", link("vless://u@e.x:443?type=kcp")),
        (
            "vless-xhttp-xray",
            link("vless://u@e.x:443?type=xhttp&security=tls"),
        ),
        (
            "vless-grpc-slash-path-xray",
            link("vless://u@e.x:443?type=grpc&serviceName=/svc&security=tls"),
        ),
        (
            "vless-ws-accept-proxy-xray",
            link("vless://u@e.x:443?type=ws&security=tls&acceptProxyProtocol=1"),
        ),
        // trojan flow forces xray.
        (
            "trojan-flow-xray",
            link("trojan://pw@e.x:443?security=tls&flow=xtls-rprx-vision"),
        ),
        // shadowsocks cipher routing (both directions) + TLS forcing.
        ("ss-plain-xray", link("ss://plain:pw@h.ex:443#x")),
        (
            "ss-ietf-chacha-singbox",
            link("ss://chacha20-ietf-poly1305:pw@h.ex:443#x"),
        ),
        (
            "ss-2022-blake3-singbox",
            link("ss://2022-blake3-aes-128-gcm:pw@h.ex:443#x"),
        ),
        (
            "ss-aes-gcm-tcp-default-xray",
            link("ss://aes-256-gcm:pw@h.ex:443#x"),
        ),
        (
            "ss-aes-gcm-tls-singbox",
            link("ss://aes-256-gcm:pw@h.ex:443?security=tls&sni=h.ex#x"),
        ),
        // TLS / Reality Xray-only fields.
        (
            "vless-reality-pqv-xray",
            link("vless://u@e.x:443?type=tcp&security=reality&pbk=PK&sni=s&pqv=Q"),
        ),
    ];

    // Struct-only fields not carried on share links.
    let mut vless_packetaddr = link("vless://u@e.x:443?type=tcp&security=tls");
    if let Profile::Vless(v) = &mut vless_packetaddr {
        v.packet_encoding = PacketEncoding::Packetaddr;
    }
    cases.push(("vless-packetaddr-singbox", vless_packetaddr));

    let mut vmess_padding = empty(Protocol::Vmess);
    if let Profile::Vmess(v) = &mut vmess_padding {
        v.vmess_global_padding = true;
    }
    cases.push(("vmess-global-padding-singbox", vmess_padding));

    let mut ws_heartbeat = link("vless://u@e.x:443?type=ws&security=tls");
    if let Profile::Vless(v) = &mut ws_heartbeat
        && let Transport::Ws(w) = &mut v.transport
    {
        w.heartbeat_period = 5;
    }
    cases.push(("vless-ws-heartbeat-xray", ws_heartbeat));

    let mut grpc_ping = link("vless://u@e.x:443?type=grpc&serviceName=svc&security=tls");
    if let Profile::Vless(v) = &mut grpc_ping
        && let Transport::Grpc(g) = &mut v.transport
    {
        g.ping_timeout = 10;
    }
    cases.push(("vless-grpc-ping-timeout-singbox", grpc_ping));

    // Per-profile core_type override (not a forced case): resolve_core honours it.
    let mut override_singbox = link("vless://u@e.x:443?type=tcp&security=tls");
    if let Profile::Vless(v) = &mut override_singbox {
        v.meta.core_type = Some(CoreEngine::SingBox);
    }
    cases.push(("vless-core-type-override-singbox", override_singbox));

    let s = AdvancedSettings::default();
    cases
        .into_iter()
        .map(|(name, profile)| {
            let expected = resolve_core(&profile, &s);
            CoreResolutionFixture {
                name,
                profile,
                expected,
            }
