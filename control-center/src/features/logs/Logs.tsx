...
type LogTarget = "service" | "xray" | "singbox" | "tun2socks" | "proxy_control";
...
export default function Logs({ onClose }: { onClose: () => void }) {
  ...
  const t = useT();
  const [target, setTarget] = useState<LogTarget>("service");
      ...
      <Sheet open title={t("logs.title")} onClose={onClose}>
        ...
        >
          <div>
            ...
            >
              <option value="service">{t("logs.target.service")}</option>
              ...
              <option value="proxy_control">{t("logs.target.proxyControl")}</option>
            </select>
          </div>
          <div>
