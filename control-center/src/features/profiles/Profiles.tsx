...
import { profileSearchText } from "../../lib/profile";
...
export default function Profiles({ onOpenEditor }: { onOpenEditor: (id: string | "new") => void }) {
  ...
  let list = profiles;
  if (groupFilter !== "all") list = list.filter((profile) => profile.groupId === groupFilter);
  if (query.trim()) {
  ...
  list.forEach((profile) => {
    const groupProfiles = byGroup[profile.groupId] ?? [];
    groupProfiles.push(profile);
    byGroup[profile.groupId] = groupProfiles;
  });
  ...
  async function importProfilesFromText(text: string) {
    ...
    addProfiles(parsed.map((profile) => ({ ...profile, groupId: importGroup })));
    setImportText("");
    setImportOpen(false);
    setAddOpen(false);
    return true;
  ...
  async function openProfileQr(profile: Profile) {
    try {
      ...
      setQrPayload({ title: profile.remarks || t("qr.show.title"), text: link });
    } catch {
      notify(t("profiles.qr.unsupported"));
    }
  }
      ...
      {sheetProfile && (
        <Suspense fallback={null}>
          <ProfileActionsSheet
            ...
            onPing={(profile) => {
              void pingProfile(profile.id);
              closeSheetProfile();
            }}
            onRealPing={(profile) => {
              void realPingProfile(profile.id);
              closeSheetProfile();
            }}
            onSpeedTest={(profile) => {
              void speedTestProfile(profile.id);
              closeSheetProfile();
            }}
            onDelete={(profile) => {
